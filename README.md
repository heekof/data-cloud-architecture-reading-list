...
## Start with Architect Gym

Turn a reading into an architecture decision: **one challenge → your initial
answer → cited evidence and guided critique → a revised, saved decision**.
Architect Gym is the main direction of this project. The first milestone is
five validated readings and one worked personal decision.

With Node.js and SQLite 3.35+ with FTS5 available:
...
Open <http://127.0.0.1:4317>. Architect Gym opens by default. Save your initial
answer to the ownership challenge before revealing the coaching prompts, then
record your critique, revised decision and reconsideration triggers. **Revue
qualité** lets you validate the five nominated readings; **Explorer** searches
the existing collection.

The first version provides guided self-critique and a 35-minute Kindle preview
pack. It does not generate an AI assessment or mark readings as understood.
[Workflow, saved records and limits](#architect-gym-from-reading-to-a-decision).
...
- `src/architect-gym.mjs` and `web/search/gym.js` — the guided decision workflow.
- `gym/examples/` — an illustrative worked decision, separate from personal progress.
- `gym/sessions/` — private local decision revisions in JSON and Markdown
  (git-ignored; back up separately).
- `src/content-quality.mjs` — shared live content fingerprints and quality evaluation.
...
- `quality.status: needs_review` means a source or Markdown is missing, Markdown
  has no words, a reviewer has rejected the source/extraction, or a previous
  approval is no longer bound to the current content.
...
`quality_review.notes`. Use `pending` for an outstanding review. Approve in the review UI after comparing the text with an adequate source. Then run `npm run metadata:sync` if editing files directly.
...
Approvals are bound to `reviewed_source_hash`, `reviewed_markdown_hash`, and `reviewed_at`. Approve through the review UI to record these fields. Source or Markdown byte changes automatically require another review; the previous decision is retained in `quality_review.history`. Older approvals without fingerprints also require review. Metadata-only edits do not revoke approval. Rejected sources stay excluded until explicitly reviewed again.
...
and empty placeholders are not substituted for article text. SQLite FTS5 indexes `articles/*/article.md` and joins metadata by article ID. Embeddings are not implemented.
...
4. **Review quality.** Compare the Markdown with its PDF, checking the beginning,
   ...
   **Revue qualité** interface, approve the exact content reviewed or reject it
   with an explanation. Leave unverified content pending. Approval is a review
   decision, not an automatic consequence of successful extraction; source or
   Markdown changes automatically invalidate it and retain the previous decision.
...
1. **Trust the existing corpus.** Validate five nominated readings against their
   original sources. Keep content-bound approvals and consistent export rules.
2. **Retrieve evidence reliably.** Use document-level full-text matching with
   exact passage citations; maintain realistic cross-section search regressions.
3. **Practice decisions in Architect Gym.** Complete the ownership session and
   save an evidence-backed revision with explicit reconsideration triggers.
4. **Extend proven exercises.** Add scenarios and reading packs after using the
   first session. Consider AI-assisted critique only with explicit evaluation of
   its grounding and usefulness.

Plain files, Node modules and SQLite remain the architecture. More article
collection, embeddings and a generic chatbot are not the next milestone.

## Optional future capability: local RAG

The following is an exploratory design, not an implemented feature or the
current delivery priority. It could eventually support evidence-backed critique
inside Architect Gym once the corpus and exercises are validated.
...
qualité de la même évaluation du contenu que la file de revue, et les compteurs du Markdown réellement lu. La recherche actualise cet index à chaque requête pour prendre en compte les changements locaux.
...
et paginés. Tous les termes sont recherchés ensemble dans le document complet, puis les passages pertinents sont classés avec les termes en OU. Les numéros de ligne restent ceux du Markdown source.
...
1. **À vérifier** : source manquante, texte absent/vide, source rejetée ou
   approbation périmée après modification du contenu.
2. **Non revus** : texte disponible mais pas encore vérifié par une personne.
3. **Validés** : revue humaine positive liée aux empreintes actuelles, sans
   problème détecté.
...
Dans le lecteur, **EPUB vérifié** exige une approbation liée au contenu actuel et une édition dont les empreintes correspondent au texte, aux sources et aux ressources. Sinon, **Aperçu EPUB non vérifié** reconstruit un aperçu depuis le texte actuel, avec un avertissement embarqué. Un texte absent, vide ou rejeté interdit les deux téléchargements ; les anciens fichiers restent sur disque. Après mise à jour du code, redémarrez le serveur de recherche.
...
Les anciennes éditions non suivies ou périmées restent préservées, mais ne sont pas proposées comme éditions vérifiées.
...
## Architect Gym: from reading to a decision

Architect Gym is the default view of `npm run search` at <http://127.0.0.1:4317>.
The existing search, quality queue and discovery tools remain available.

1. Read the ownership challenge and save your initial decision before seeing the coaching prompts.
2. Compare assumptions, alternatives, failure modes and operations against three cited excerpts from two readings. Each excerpt records its source/Markdown hashes, exact lines and extraction-quality state.
3. Write your critique, revised decision and reconsideration triggers. The original answer and evidence snapshot are retained unchanged.
4. Resume a session or download its Markdown decision note.

This first version offers **guided self-critique**, not an LLM-generated assessment or score. It does not send your answer or articles to an external service. A complete [illustrative decision](gym/examples/ownership-decision.md) is available; it is not counted as your learning progress. Read it only after attempting the challenge.

Sessions are plain files under `gym/sessions/<uuid>/<revision>/decision.json` and `decision.md`. Revisions are published together in a new directory; stale saves are rejected. Keep these files to retain your learning history. They may contain professional context: review them before deliberately committing or sharing them. The folder is ignored by Git by default. Extraction approvals remain in article metadata, independently of session completion.

The milestone panel tracks **five nominated readings validated and one completed personal decision**. It does not approve readings automatically. Compare each nominated Markdown with the original source using the quality queue, including completeness, tables, diagrams and reading order. Current unreviewed readings are clearly labelled.

The **35-minute Kindle pack** contains the challenge, up to 4,000 words of explicitly labelled reading excerpts, and recall questions. Timebox: 5 minutes initial answer, 20 minutes reading, 10 minutes recall/revision. It contains no suggested solution. It is a preview with source-quality warnings; rejected/missing readings block generation. Kindle device rendering and Send to Kindle delivery remain manual checks.

Search regression examples include `catalog governance` across separate sections, exact phrases, accents, metadata filters, source replacement and stale citations. On this small corpus the freshness check hashes local source bytes on each search; this prioritizes consistency over large-corpus throughput.
