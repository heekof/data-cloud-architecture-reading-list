...
The positions expressed in this paper are
personal opinions and do not in any way reflect
the positions of my employer Amazon.com.
...
serializability. Personally, I have invested a nontrivial portion of my career as a strong advocate
...
machines and their locations. The lower layer is scaleaware in that it understands this mapping. We are
presuming that the lower layer provides a scaleagnostic programming abstraction to the upper layer4.
Using this scale-agnostic programming abstraction, the
upper layer of application code is written without
worrying about scaling issues. By sticking to the scaleagnostic programming abstraction, we can write
...
for the Scale-Agnostic
Programming Abstraction

evolve to become new platforms or middleware which
simplify the creation of scale-agnostic applications
(similar to the past scenarios when CICS and other TPMonitors evolved to simplify the creation of
...
To be clear, this is conceptually assuming tens of
thousands or hundreds of thousands of machines. Too
many to make them behave like one “big” machine.
3
Scaling at N log N for some big log would be really
nice…

4

Google’s MapReduce is an example of a scale-agnostic
programming abstraction.
...
cluster).
Each entity has a unique identifier or key. An entitykey may be of any shape, form, or flavor but it
...
7
This is excluding replication for high-availability which
will not change the presumption of disjoint scopes of
tra`nsactional serializability.

8

I am a big fan of “exactly-once in-order” messaging but
...
recipient entity comprises both upper-layer (scaleagnostic) business logic and the durable data
representing its state which is stored and accessed by
the entity-key.
Recall the assumption that messages are delivered “atleast-once”. This means that the recipient entity must
...
Entities represent disjoint sets of data. Each datum
...
scale-agnostic API for the business-logic. The upperlayer scale-agnostic business logic simply addresses the
message to the entity-key that identifies the durable
state known as the entity.

135

that the specified order is ready-to-ship. Frequently,
...
for the entity-keys, most of the time the adjacent keyvalues resides on the same machine but once in a while
...
share transactional scopes.
Entities never share
...
We see customer-ids, social-security-numbers, productSKUs, and other unique identifiers all the time within
...
One of our assumptions is that the emerging upperlayer is scale-agnostic and the lower-layer decides how
...
Entities are spread across scopes of serializability
using either hashing or key-range partitioning.
...
Recall the premise that almost-infinite scaling causes
...
cannot be known to reside within a single scope of
...
Different keys (primary entity-key versus alternate
keys) cannot be collocated or updated atomically.

In this section, we consider the means to connect
independent entities using messages.
We examine
...
entity-key. Second, we access the entity using the entitykey. This is very much like inside a relational database as
...
data.
The functionality previously implemented as
...
for the entity and, hence, the destination for the message
...
Hence, we consider what the poor developer of the scaleagnostic application must implement.

11

It is common that scale-aware applications are not
...
12
Alternatively, you could forget about collocation and
...
In the database community, we frequently use this
...
14
Note that it is hard (but certainly doable) to build the
...
than the entity.
Migrating the state for duplicate
...
general confusion experience by that entity.
As
...
uncertainty. It is normal to proceed through life with ever
...
captured and some high-end applications evolved into TPmonitors. Eventually, these patterns were repeated in the
...
unacceptable pressure on availability.
Instead, the
